...
import com.appirio.service.member.events.MemberUpdateEventHandler;
 ...
 */
public class MemberUpdateEventHandlerTest {
    ...
    public void init() throws InvocationTargetException, NoSuchMethodException,
        ...
        when(memberProfileDAO.getMemberProfile(anyInt())).thenReturn(memberProfile);
        PowerMockito.doNothing().when(memberProfileDAO).updateMemberProfile(memberProfile, false);
    }

    @Test
    public void testMemberUpdateEventHandler() throws InvocationTargetException, NoSuchMethodException,
            InstantiationException, SupplyException, IllegalAccessException {

        MemberUpdateEventHandler memberUpdateEventHandler = new MemberUpdateEventHandler(memberProfileDAO, eventProducer);
        memberUpdateEventHandler.handleEvent(event);
        verify(memberProfileDAO).getMemberProfile(anyInt());
